...
class TargetClient:
    def __init__(self, host: str, port: int, username: str, password: str) -> None:
        ...
        send_json(self.sock, PacketType.AUTH_REQ, {"username": username, "password": password, "role": "target"})
        p, _ = recv(self.sock)
        assert p is PacketType.AUTH_OK

        self._on_chat: Callable[[str, str], None] = lambda u, m: None
        self._on_perm_req: Callable[[dict, str], dict] = lambda req, ctrl: {"view": True}
    ...
    def on_chat(self, cb): self._on_chat = cb
    def on_perm_request(self, cb): self._on_perm_req = cb
    ...
    def _reader(self):
        while True:
            ...
            if p is PacketType.CHAT:
                ...
                send_json(self.sock, PacketType.PERM_RESPONSE,
